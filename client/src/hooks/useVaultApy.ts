...
async function calculateVaultApy(
  ...
  withdrawQueue.forEach((marketId) => {
    marketCalls.push(
      { address: morphoBlueAddress, abi: MORPHO_BLUE_ABI, functionName: 'market', args: [marketId] },
      { address: morphoBlueAddress, abi: MORPHO_BLUE_ABI, functionName: 'idToMarketParams', args: [marketId] },
      { address: morphoBlueAddress, abi: MORPHO_BLUE_ABI, functionName: 'position', args: [marketId, vaultAddress] }
    );
  });

  const marketResults = await client.multicall({ contracts: marketCalls, allowFailure: false });

  // Parse results: For each market, extract state, params, supplyShares
  const marketData: { state: MarketState; params: MarketParams; supplyShares: bigint }[] = [];
  for (let i = 0; i < withdrawQueue.length; i++) {
    const idx = i * 3;
    const marketResult = marketResults[idx] as [bigint, bigint, bigint, bigint, bigint, bigint]; // Tuple from ABI
    const paramsResult = marketResults[idx + 1] as [Address, Address, Address, Address, bigint];
  ...
  if (borrowCalls.length > 0) {
    const borrowResults = await client.multicall({ contracts: borrowCalls, allowFailure: false });
    borrowRates = borrowResults as bigint[];
  }

  // Now compute per-market data using borrowRates (index matches filtered)
  let borrowRateIdx = 0;
  const processedMarketData = marketData.map(({ state: staleState, params, supplyShares }) => {
    let apy = 0n;
    ...
    const allocation = toAssetsDown(supplyShares, state.totalSupplyAssets, state.totalSupplyShares);
    return { supplyApy: apy, allocation };
  });
...
async function fetchMerklAprForVault(vaultAddress: Address): Promise<{ totalApr: number | null; opportunity?: MerklOpportunity }> {
  try {
    ...
    const response = await axios.get(`https://api.merkl.xyz/v4/opportunities/?${params.toString()}`, {
      timeout: 5000, // Efficient: short timeout
      headers: { 'Content-Type': 'application/json' },
    });

    // Filter for matching vault by identifier (address)
    const matchingOpportunity = response.data.find((item: any) => item.identifier.toLowerCase() === vaultAddress.toLowerCase()) as MerklOpportunity | undefined;
...
export const useVaultApy = (tokenAddress: Address, morphoBlueAddress: Address) => {
  ...
  useEffect(() => {
    ...
    const fetchApy = async () => {
      try {
        ...
        const merklPromise = fetchMerklAprForVault(vaultAddress[tokenAddress as keyof typeof vaultAddress] as `0x${string}`);
        const onChainPromise = calculateVaultApy(client, vault as `0x${string}`, morphoBlueAddress);

        // Await both in parallel for efficiency
        const [merklData, rawNativeApy] = await Promise.all([merklPromise, onChainPromise]);
  ...
  useEffect(() => {
    ...
    const fetchFees = async () => {
      try {
        ...
        const vault = await Vault.fetch(vaultAddress[tokenAddress as keyof typeof vaultAddress] as `0x${string}`, client);
        // console.log("VAULT", vault)
  ...
  return { nativeApy, totalApr, fees, loading, error };
};